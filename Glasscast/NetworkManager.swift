...
actor NetworkManager {
    ...
    func post<T: Codable>(url: String, queryParameters: [String: Any]? = nil, body: Data) async -> T? {
    ...
    func put<T: Codable>(url: String, queryParameters: [String: Any]? = nil, body: Data) async -> T? {
    ...
    private func request<T: Codable>(
        ...
        if let accessToken = await KeychainHelper.get("accessToken") {
            request.setValue("Bearer \(accessToken)", forHTTPHeaderField: "Authorization")
        }

        await logger.debug("Preparing request: method=\(method.rawValue, privacy: .public), url=\(url, privacy: .public), queryParameters=\(String(describing: queryParameters), privacy: .public), body=\(String(describing: body?.count), privacy: .public)")
        ...
        while attempt <= retryCount {
            do {
                ...
                if shouldRetryFlag && attempt <= retryCount {
                    ...
                    await logger.error("Request failed on attempt \(attempt): \(error.localizedDescription)")
                }
            }
        }
    ...
    private func performRequest<T: Codable>(_ request: URLRequest) async throws -> T? {
        ...
        guard let httpResponse = response as? HTTPURLResponse else {
            await logger.error("Response is not HTTPURLResponse.")
            return nil
        }

        await logger.debug("Received response: statusCode=\(httpResponse.statusCode, privacy: .public), url=\(httpResponse.url?.absoluteString ?? "unknown", privacy: .public)")

        guard (200...299).contains(httpResponse.statusCode) else {
            throw URLError(.badServerResponse)
        }

        return await data.decodeUsingJSONDecoder()
    }
